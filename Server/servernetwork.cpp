...
ServerNetwork::ServerNetwork(unsigned short port, bool rawMode = false) : listen_port(port)
{
    ...
    if (rawMode)
        println("Warning, raw mode enabled");
...
void ServerNetwork::ConnectClients(std::vector<sf::TcpSocket *> *client_array)
{
    while (true)
    {
        ...
        if (listener.accept(*new_client) == sf::Socket::Done)
        {
            ...
            println("Added client " << new_client->getRemoteAddress() << ":" << new_client->getRemotePort() << " on slot " << client_array->size());
        }
        else
        {
            println("Server listener error, restart the server");
            delete (new_client);
            break;
        }
    }
...
{
    println("Client " << socket_pointer->getRemoteAddress() << ":" << socket_pointer->getRemotePort() << " disconnected, removing");
    ...
    for (size_t iterator = 0; iterator < client_array.size(); iterator++)
    {
        ...
        if (client->getRemoteAddress() != exclude_address || client->getRemotePort() != port)
        {
            if (client->send(packet) != sf::Socket::Done)
                println("Could not send packet on broadcast");
        }
    }
}
...
{
    char received_data[MAX_RAW_DATA];
    ...
    else if (received_bytes > 0)
    {
        ...
        println(client->getRemoteAddress().toString() << ":" << client->getRemotePort() << " '" << received_data << "'");
    }
}
...
void ServerNetwork::ReceivePacket(sf::TcpSocket *client, size_t iterator)
{
    ...
    else if (packet.getDataSize() > 0)
    {
        ...
        println(client->getRemoteAddress().toString() << ":" << client->getRemotePort() << " '" << received_message << "'");
    }
}
...
void ServerNetwork::ManagePackets()
{
    while (true)
    {
        ...
        {
            if (rawMode == true)
                ReceiveRawData(client_array[iterator], iterator);
            else
